...
use simplelog::{self, CombinedLogger, LevelFilter, TerminalMode, TermLogger, WriteLogger};
...
fn blank_porep_poseidon_circuit(
    ...
    let public_params =
        <StackedCompound<PoseidonHasher, Sha256Hasher> as CompoundProof<_, StackedDrg<PoseidonHasher, Sha256Hasher>, _>>::setup(
            &setup_params,
        )
        .unwrap();
...
fn verify_param_transitions(param_paths: &[&str], contribution_hashes: &[[u8; 64]]) {
    ...
    for (param_pair, provided_contribution_hash) in param_paths
        .windows(2)
...
/*
fn verify_param_transistions_interactively() {
    let start_total = Instant::now();

    use std::io::{self, Read, Write};

    let mut stdin = String::new();
    let mut before_params_opt: Option<phase2::MPCParameters> = None;
    let mut before_path_opt: Option<String> = None;

    loop {
        let before_path = match before_path_opt.take() {
            Some(before_path) => before_path,
            None => {
                io::stdout().write_all(b"'before' params filename: ");
                io::stdout().flush().unwrap();
                io::stdin().read_line(&mut stdin).unwrap();
                stdin.trim_end().to_string()
            }
        };

        println!("'before' path: {}", before_path);

        // Check that file exists:
        let before_file = File::open(&before_path).unwrap();

        // If file doesn't exist, re-type filename:

        stdin.clear();
        io::stdout().write_all(b"'after' params filename: ");
        io::stdout().flush().unwrap();
        io::stdin().read_line(&mut stdin).unwrap();
        let after_path = stdin.trim_end().to_string();

        println!("'after' params: {}", after_path);

        // Check that file exists:
        let after_file = File::open(&after_path).unwrap();
        // If file doesn't exist, re-type filename:

        stdin.clear();
        io::stdout().write_all(b"contribution hash: ");
        io::stdout().flush().unwrap();
        io::stdin().read_line(&mut stdin).unwrap();
        let hex_str = stdin.trim_end();
        let mut provided_contribution_hash = [0u8; 64];
        let contribution_hash_vec = hex::decode(&hex_str)
            .expect(&format!("contribution hash is not a valid hex string: {}", hex_str));
        provided_contribution_hash.copy_from_slice(&contribution_hash_vec[..]);
        println!("contribution hash: {}", hex::encode(&provided_contribution_hash[..]));
        ...
        let before_params = {
            info!("reading 'before' params from disk: {}", before_path);
            let mut reader = BufReader::with_capacity(1024 * 1024, before_file);
            let start = Instant::now();
            let before_params = phase2::MPCParameters::read(&mut reader, true).unwrap();
            info!("successfully read 'before' params from disk, dt_read={}s", start.elapsed().as_secs());
            before_params
        };

        let after_params = {
            info!("reading 'after' params from disk: {}", after_path);
            let mut reader = BufReader::with_capacity(1024 * 1024, after_file);
            let start = Instant::now();
            let after_params = phase2::MPCParameters::read(&mut reader, true).unwrap();
            info!("successfully read 'after' params from disk, dt_read={}s", start.elapsed().as_secs());
            after_params
        };
        ...
        let calculated_contribution_hash =
            phase2::verify_contribution(&before_params, &after_params).expect("invalid transition");
        ...
        before_params_opt = Some(after_params);
        before_path_opt = Some(after_path);
    }
...
fn main() {
    // verify_param_transistions_interactively();
        ...
        match subcommand {
            "new" => {
                let proof = if matches.is_present("porep") {
                    ...
                    Proof::ElectionPost
                } /*else if matches.is_present("fallback-post") {
                    Proof::FallbackPost
                } */else {
                    // TODO: change after FallbackPoSt is added.
                    // Proof::FallbackPost
                    unreachable!()
                };
                ...
                */
                ...
                let sector_size = if matches.is_present("2kib") {
                    ...
                    SECTOR_SIZE_32_GIB
                } /*else if matches.is_present("64gib") {
                    SECTOR_SIZE_64_GIB
                } */else {
                    // TODO: change after 64GiB sectors are added.
                    // SECTOR_SIZE_64_GIB
                    unreachable!()
                };
